  ...
  if (viaToggle && !viaQuery) {
    ...
    requestAnimationFrame(function () {
      ...
      if (isDeck) {
        ...
        // Find any fixed/sticky bar pinned to the top edge, by GEOMETRY, not by
        // tag name. The old version looked only for nav/header/[role=banner];
        // a page whose top bar was <div class="topbar"> — which satisfies every
        // written instruction — was invisible to it, so the launcher stayed at
        // top:8px and silently swallowed clicks on whatever sat beneath it.
        // Shipped that way in 3 of 5 real lessons. Never narrow this to tags.
        var navBottom = 0;
        var vw = window.innerWidth || document.documentElement.clientWidth;
        Array.prototype.forEach.call(document.body.querySelectorAll("*"), function (el) {
          if (el === launch || launch.contains(el)) return;
          var s = getComputedStyle(el);
          if (s.position !== "fixed" && s.position !== "sticky") return;
          if (s.visibility === "hidden" || s.display === "none") return;
          var r = el.getBoundingClientRect();
          // touching the top edge, meaningfully wide, and not the whole viewport
          if (r.top <= 4 && r.bottom > 0 && r.width >= vw * 0.4 && r.height <= 200) {
      ...
      // Belt and braces: whatever the heuristic decided, verify the launcher is
      // not covering something clickable. Geometry can still collide on narrow
      // viewports, which is why this bug reproduced for one reader and not
      // another — top-bar contents reflow with window width and zoom.
      requestAnimationFrame(function () {
        var lr = launch.getBoundingClientRect();
        var hits = document.elementsFromPoint
          ? document.elementsFromPoint(lr.left + lr.width / 2, lr.top + lr.height / 2)
          : [];
        var covering = false;
        for (var i = 0; i < hits.length; i++) {
          var h = hits[i];
          if (h === launch || launch.contains(h)) continue;
          if (h.closest && h.closest('button,a[href],input,select,textarea,[role=button],[onclick],[tabindex]:not([tabindex="-1"])')) {
            covering = true;
            break;
          }
        }
