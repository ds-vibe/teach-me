...
**Research.** Don't research if it's an established topic unlikely to have changed recently. If it's possibly dynamic, do 2–3 fetches. **Established is not the same as static:** when the lesson quotes a codified or versioned source — a statute, rule, standard, spec, or edition — verify the version in force and date-stamp it, however old the topic is. Settled topics can have updates too.

**One rule is rarely the whole answer.** When a source says something is allowed, required or banned, go looking for what else covers that same act — elsewhere in the same document, and in the other rules that apply to it — before you write the conclusion. Finding the part that answers your question is not the same as finding all of it, and the part you found will not tell you what you missed. A prior test taught readers an act was legal on the strength of one section that said only that it didn't breach that section; three other bodies of law bore on it.

**Let the stakes set the ceiling, not the clock.** Two or three fetches is the budget for a topic where being wrong costs a learner an argument. It is the wrong budget when a reader could act on the lesson and be harmed — medicine, law, safety, money, anything a professional would be sued over. There, go claim by claim: every number, threshold, dose, deadline and "always/never" gets checked against the primary source that governs it, and a claim you cannot source that way comes out of the lesson rather than going in hedged. Fetch until the list is done. A prior test shipped a clinical lesson whose medication section read as protocol and understated the guideline's default, because the quota ran out before the claims did.
...
**The opening commit.** Make it deliberately unanswerable — it should be missing an input the lesson exists to teach. Test it before building: name the missing input and confirm that changing it changes the answer. If the answer comes out the same either way it's a trick question, and the designated answer is wrong. Give it an escape option ("can't tell yet") authored as `esc:true`, which the runtime pins last through the shuffle — and give the same out to any question asked before its teaching. Without it, a learner who hasn't met the rule is guessing and the reveal punishes them for it. Commits are ungraded and sit outside the item budget.

**Organization is the highest-leverage decision of the build.** Before planning sections, name what the learner is actually trying to do — predict a payout, run a consult, analyze an exam question — and pick the structure that has them doing it soonest: a decision algorithm, a running scenario, a workflow, a motivating problem, a historical evolution, a causal chain. Prior tests found mis-organization is the one defect polish cannot recover: a lesson that spent its opening on lucid, accurate history still lost the reader before the part they came for. History organizes the lesson only when it is genuinely the fastest route to the working model; otherwise teach the model first and let history explain it afterward.

**Organizing Ideas and Historical Examples.** Teach using one or more compelling organizing ideas or themes. Examples: a running scenario (e.g., a company's story, a car accident); a historical causal chain explaining how a complex system evolved (e.g., incentive --> abuse --> fix --> incentive), the moves of a game. Also consider historical examples that explain the topic's development (e.g., how "due process" emerged in the Magna Carta). Use these in the learning path, though you don't need to make every question about them. If a running case carries the lesson, every section does work on the case — the learner computes, judges, or decides something about it each time, not just meets its name in the intro. Prior tests found the strongest builds worked one chart the whole way through; the weakest mentioned their scenario twice.
...
- **Do you have a results file?** Every lesson and drill ships a "Download your results" button; ask for that JSON. If they upload one, it is the calibration: the weakest objectives get the biggest batteries and the hardest ramp, strong ones get a few retention checks, and the mistakes it records become distractors. **Chain the history too:** set `TM_SLUG` to the `slug` field in the uploaded results JSON and reuse its objective labels verbatim — this works whether the source was the original lesson or an earlier drills round, so the chain continues indefinitely: in the same browser every generation appends to one attempt history, the tally spans all of it, and each download supersedes the last. A genuinely new focus area may add a new objective label; it starts its own line in the tally beside the inherited ones.
...
Lint with the flag: `node scripts/lint.mjs <file> --len <short|full> --drills`. Everything else — shell, runtime, review agents, delivery order — is unchanged.
