...
Build a high-quality HTML lesson in the user's selected topic.
...
Then, teach using the most compelling presentation of the organizing idea(s) incorporated in the learning path. If a running scenario is your theme, each section should pick up on the scenario - the learner computes, judges or decides something about it each time. The strongest builds do that, and weak ones name a scenario 1-2x and drop it. Vary the facts, never the frame: the strongest practice items are new instances inside the running scenario — the next post in the queue, the next deal of the season. Mixing fact patterns is good interleaving; mixing frames dilutes the lesson.
...
*Floor.* a short lesson needs at least 1 drawn visual that teaches, and at least 1 of the visuals must be an interaction (see below); a full lesson needs 3, of which 2 are interactions. Aim above the floor — 3–5 in a short lesson, 5–8 in a full one; the floor is what the lint will pass, not what a good build ships.
...
**Page layout.** Copy `reference/lesson-shell.html` and `reference/lesson-shell.css` — don't invent chrome. The skeleton gives you a sticky top bar (section menu, Quick Reference, pause, voice toggle), a left section rail, the closed-book final stage with its stepper and readout, a progress ledger, and a Quick Reference drawer. Fill the marked content slots, leave the rest verbatim. The CSS is responsive already. Keep the hero short — a few hundred words at most before the first interaction.
...
If you're behind, cut in this order: first the visuals and devices above the floor, then an objective and its section whole. Never thin the final, drop typed items, or skip wrong-answer narration — those are floors and the lint fails them anyway.
